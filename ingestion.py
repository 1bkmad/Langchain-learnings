...
ssl_context = ssl.create_default_context(cafile=certifi.where())   #defensive programming to avoid ssl certificate issue for connecting with Tavily and do the requests. 
os.environ["SSL_CERT_FILE"] = certifi.where()
os.environ["REQUESTS_CA_BUNDLE"] = certifi.where()


#rate limit can cause the erros and we might have to wait for some seconds before retrying the request. So, we can set the retry_min_seconds to 10 seconds to avoid the errors.
embeddings = OpenAIEmbeddings(model="text-embedding-3-small", chunk_size=50, retry_min_seconds=10, show_progress_bar=True)  
# chorma = Chroma(persist_directory="chroma_db", embedding_function=embeddings)
...
async def main():
    ...
    #         "max_depth": 5,    #max_depth = 5 means it will crawl the website up to 5 levels deep from the starting URL. it.e. 5 more clicks inside a link. Tavily best practises document for reference
    #         "extractor_depth": "advanced",
    #         }
    # )
#    all_docs = res["results"]
    ...
    log_success(f"TavilyCrawl: Successfully crawled {len(all_docs)} URLs from the site")

#Document chunking
    log_header("Document Chunking Phase")
    log_info(
        f"Text SPlitter: Processing{len(all_docs)} document with 4000 chunk size and 200 overlap", 
        Colors.YELLOW,
    )
    #Splitting documents
    ...
    #Indexing documents
    await index_documents_async(splitted_docs, batch_size = 500) # too big batch_size can cause rate limit exceed in imbedding model. Also,  according to the vectorDB, it can change
    log_header("PIPELINE COMPLETE")
    log_success("Documentation ingestiong pipeline finished successfully!")
